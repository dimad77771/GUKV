...
namespace GUKV.DataMigration
{
    ...
    public class OrganizationFinder
    {
        ...
        /// </summary>
        private CSVLog logUncategorizedOrg = new CSVLog("CSV/UncategorizedOrg.csv");

        /// <summary>
        /// Results of categorization for all organizations are written to this log
        /// </summary>
        private CSVLog logCategorization = new CSVLog("CSV/OrgCategorization.csv");
        ...
        /// <summary>
        /// Prepares the cache of organizations from the 1NF database
        /// </summary>
        /// <param name="connection1NF">Connection to the 1 NF database</param>
        /// <param name="writeLogs">If set to "true", this method will write detailed logs about categorization</param>
        public void BuildOrganizationCacheFrom1NF(FbConnection connection1NF, bool writeLogs)
        {
            mapZkpoToId.Clear();
            mapOrgByShortName.Clear();
            mapOrgByFullName.Clear();
            mapOrgProperties.Clear();
            duplicateZkpo.Clear();
            uncategorizedOrganizations.Clear();

            if (writeLogs)
            {
                // Prepare the CVS log of uncategorized organizations
                logUncategorizedOrg.WriteCell(GUKV.DataMigration.Properties.Resources.UncategorizedOrgID);
                logUncategorizedOrg.WriteCell(GUKV.DataMigration.Properties.Resources.UncategorizedOrgZKPO);
                logUncategorizedOrg.WriteCell(GUKV.DataMigration.Properties.Resources.UncategorizedOrgFullName);
                logUncategorizedOrg.WriteCell(GUKV.DataMigration.Properties.Resources.UncategorizedOrgShortName);
                logUncategorizedOrg.WriteEndOfLine();

                // Prepare the CVS log of categorization results
                logCategorization.WriteCell("OrgID");
                logCategorization.WriteCell("ZKPO");
                logCategorization.WriteCell("Full Name");
                logCategorization.WriteCell("Stripped Full Name");
                logCategorization.WriteCell("Short Name");
                logCategorization.WriteCell("Stripped Short Name");
                logCategorization.WriteEndOfLine();
            }

            // Get the required columns from the SZKPO table
            string querySelect = "SELECT KOD_OBJ, KOD_ZKPO, FULL_NAME_OBJ, SHORT_NAME_OBJ, KOD_GALUZ, KOD_VID_DIAL, KOD_FORM_VLASN, DELETED FROM SORG_1NF WHERE KOD_STAN = 1";
            ...
            using (FbCommand commandSelect = new FbCommand(querySelect, connection1NF))
            {
                using (FbDataReader reader = commandSelect.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        string zkpo = reader.IsDBNull(1) ? "" : reader.GetString(1).Trim();
                        string fullName = reader.IsDBNull(2) ? "" : reader.GetString(2).Trim();
                        string shortName = reader.IsDBNull(3) ? "" : reader.GetString(3).Trim();
                        object industryCode = reader.IsDBNull(4) ? null : reader.GetValue(4);
                        object subIndustryCode = reader.IsDBNull(5) ? null : reader.GetValue(5);
                        object ownershipCode = reader.IsDBNull(6) ? null : reader.GetValue(6);
                        object deleted = reader.IsDBNull(7) ? null : reader.GetValue(7);

                        if (dataId is int)
                        {
                            int organizationId = (int)dataId;

                            // Create full cache of organizations
                            ...
                            if (!info.deleted)
                            {
                                ...
                                // Create a cache by organization names
                                ...
                                if (info.category == OrgCategory.Undefined)
                                {
                                    ...
                                    if (writeLogs)
                                    {
                                        logUncategorizedOrg.WriteCell(organizationId);
                                        logUncategorizedOrg.WriteCell(zkpo);
                                        logUncategorizedOrg.WriteCell(fullName);
                                        logUncategorizedOrg.WriteCell(shortName);
                                        logUncategorizedOrg.WriteEndOfLine();
                                    }
                                }
                                ...
                                if (writeLogs)
                                {
                                    // Write categorization results to the global log
                                    logCategorization.WriteCell(info.organizationId1NF);
                                    logCategorization.WriteCell(info.zkpoCode);
                                    logCategorization.WriteCell(info.fullName);
                                    logCategorization.WriteCell(info.fullNameStripped);
                                    logCategorization.WriteCell(info.shortName);
                                    logCategorization.WriteCell(info.shortNameStripped);
                                    logCategorization.WriteEndOfLine();
                                }
                                */
                            }
        ...
        /// <summary>
        /// Prepares the cache of organizations from the "Rosporyadzhennya" database
        /// </summary>
        /// <param name="connection1NF">Connection to the "Rosporyadzhennya" database</param>
        /// <param name="writeLogs">If set to "true", this method will write detailed logs about categorization</param>
        public void BuildOrganizationCacheFromNJF(FbConnection connectionNJF, bool writeLogs)
        {
            mapZkpoToId.Clear();
            mapOrgByShortName.Clear();
            mapOrgByFullName.Clear();
            ...
            // Get the required columns from the SZKPO table
            string querySelect = "SELECT KOD, ZKPO, NAMEP, NAME, OT, NULL, FORMVL, DT_END FROM SZKPO WHERE STAN = 1";
            ...
            using (FbCommand commandSelect = new FbCommand(querySelect, connectionNJF))
            {
                using (FbDataReader reader = commandSelect.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        object subIndustryCode = reader.IsDBNull(5) ? null : reader.GetValue(5);
                        object ownershipCode = reader.IsDBNull(6) ? null : reader.GetValue(6);
                        object deleted = reader.IsDBNull(7) ? (int)0 : (int)1;
        ...
        /// <summary>
        /// Prepares the cache of organizations from the SQL Server database
        /// </summary>
        /// <param name="connection">Connection to the SQL Server</param>
        /// <param name="writeLogs">If set to "true", this method will write detailed logs about categorization</param>
        public void BuildOrganizationCacheFromSqlServer(SqlConnection connection, bool writeLogs)
        {
            mapZkpoToId.Clear();
            mapOrgByShortName.Clear();
            mapOrgByFullName.Clear();
            ...
            // Get the required columns from the SZKPO table
            string querySelect = "SELECT id, zkpo_code, full_name, short_name, industry_id, occupation_id, form_ownership_id, is_deleted FROM organizations WHERE master_org_id IS NULL and (is_deleted is null or is_deleted = 0)";
        ...
        private OrgCategory CategorizeOrganization(ref string orgName)
        {
            ...
            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameZHBK))
            {
                return OrgCategory.ZHBK;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameZHEK))
            {
            ...
            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNamePobut))
            {
                return OrgCategory.Pobutova;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameUpr))
            {
                return OrgCategory.Upravlinnya;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameAssoc))
            {
                return OrgCategory.Association;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameBlago))
            {
                return OrgCategory.Blagodiyna;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameFOP))
            {
                return OrgCategory.FOP;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameTOV))
            {
                return OrgCategory.TOV;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNamePainters))
            {
                return OrgCategory.Painter;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameAdvBureau))
            {
                return OrgCategory.AdvocateBureau;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameOSBB))
            {
                return OrgCategory.OSBB;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameComBank))
            {
                return OrgCategory.Bank;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNamePAT))
            {
                return OrgCategory.PAT;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameZAT))
            {
                return OrgCategory.ZAT;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameAdvocate))
            {
                return OrgCategory.Advocate;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameGromad))
            {
                return OrgCategory.Gromadska;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameKP))
            {
                return OrgCategory.Komunalna;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameKoop))
            {
                return OrgCategory.Kooperativ;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameSP))
            {
                return OrgCategory.Spilna;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNameAT))
            {
                return OrgCategory.Akcionerna;
            }

            if (OrgNameMatchesCategory(ref orgName, Properties.Settings.Default.OrgNamePP))
            {

CREATE VIEW [dbo].[view_arenda_base]
AS
SELECT [an].[id] AS 'arenda_id'
	  ...
	  ,[an].[rent_rate_uah] AS 'rent_rate_uah'
      ,[an].[rent_square]
      ...
      ,an.[date_expert]
      , (CASE WHEN an.agreement_state = 1 THEN 'Договір діє' ELSE CASE WHEN an.agreement_state = 2 THEN 'Договір закінчився, але заборгованність не погашено' ELSE CASE WHEN an.agreement_state = 3 THEN 'Договір закінчився, оренда продовжена іншим договором' ELSE '' END END END) AS 'agreement_active_s'
      ,r.[short_name] AS 'payment_type_obj'
      ,r.rental_rate
      ,an.invent_no_agr_obj
      ,bal.invent_no_bti as invent_no_balans
      ,an.note_status_id
	  ,rf.[short_name] AS 'factich_vikorist'
	  ,[org_holder].form_gosp as 'form_gosp'
	  ,r.[full_name] AS 'payment_type_obj_full'
	  ,an.ref_balans_id
FROM
    ...
    left join dict_rental_rate r on an.payment_type_id_obj = r.id
	left join dict_factich_vikorist rf on an.factich_vikorist_id = rf.id

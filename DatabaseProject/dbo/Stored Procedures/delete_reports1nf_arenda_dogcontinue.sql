CREATE PROCEDURE [dbo].[delete_reports1nf_arenda_dogcontinue] 
	@id int
AS
BEGIN
	...
	--if exists (select * from reports1nf_arenda_dogcontinue A where A.id = @id and substring(isnull(komis_protocol,''),1,1) not in ('','0'))
	--begin
	...
	DELETE FROM [reports1nf_arenda_dogcontinue] WHERE [id] = @id;
END

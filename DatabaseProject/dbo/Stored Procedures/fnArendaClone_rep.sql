CREATE   PROCEDURE [dbo].[fnArendaClone_rep]
(	
	@arenda_id INTEGER,
	@rep_id INTEGER,
	@nrep_id INTEGER,
...
BEGIN
	...
	set @minExistingId = 0
	SELECT @minExistingId = MIN(id) FROM reports1nf_arenda WHERE report_id = @nrep_id
	...
	where A.id = @arenda_id and A.report_id = @rep_id
	...
	from [reports1nf_arenda] A where A.id = @arenda_id and A.report_id = @rep_id
	...
	select @newAgreementId, building_id, org_balans_id, org_renter_id, org_giver_id, balans_id, rent_year, object_kind_id, purpose_group_id, purpose_id, purpose_str, name, is_privat, update_src_id, agreement_kind_id, @agreement_date, @agreement_num, agreement_str, floor_number, num_people, cost_narah, cost_payed, cost_debt, cost_agreement, cost_expert_1m, cost_expert_total, pidstava, pidstava_date, pidstava_num, pidstava_fact, pidstava2, pidstava_num2, pidstava_date2, pidstava_display, rent_start_date, rent_finish_date, rent_actual_finish_date, rent_rate, rent_rate_uah, rent_square, priznak_1nf, debt_timespan, order_num, order_date, order_no2, rishennya_id, is_inactive, inactive_date, is_deleted, del_date, date_expert, is_subarenda, privat_kind_id, num_primirnikiv, date_expl_enter, num_akt, date_akt, num_bti, date_bti, svidotstvo_serial, svidotstvo_num, svidotstvo_date, payment_type_id, arch_id, @modified_by, @modify_date, note, agreement_state, is_insured, insurance_start, insurance_end, insurance_sum, @nrep_id, @new_unique_id, null, is_loan_agreement, is_valid, validation_errors
	from [reports1nf_arenda] A where A.id = @arenda_id and A.report_id = @rep_id

	insert into [reports1nf_arenda_payments] (report_id, arenda_id) VALUES (@nrep_id, @newAgreementId)
	...
	select @newAgreementId, rishen_id, ord, modified_by, modify_date, doc_num, doc_date, doc_dodatok, doc_punkt, purpose_str, rent_square, decision_id, doc_raspor_id, pidstava, @nrep_id
	from [reports1nf_arenda_decisions] A where A.arenda_id = @arenda_id and A.report_id = @rep_id
	...
	select @newAgreementId, purpose_group_id, purpose_id, purpose_str, rent_square, modify_date, modified_by, note, rent_rate, rent_rate_uah, cost_narah, cost_agreement, is_deleted, del_date, cost_expert_total, date_expert, payment_type_id, invent_no, note_status_id, @nrep_id
	from [reports1nf_arenda_notes] A where A.arenda_id = @arenda_id and A.report_id = @rep_id

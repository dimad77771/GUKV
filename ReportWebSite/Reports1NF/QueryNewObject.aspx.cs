...
public partial class Reports1NF_Report1NFFreeMap : System.Web.UI.Page
{
	...
	protected string selected_lon;
	protected string selected_lat;
	...
	protected void Page_Load(object sender, EventArgs e)
    {
		...
		selected_fs_id = "0";
		selected_lon = (Request.QueryString["lon"] != null ? Request.QueryString["lon"] : "null");
		selected_lat = (Request.QueryString["lat"] != null ? Request.QueryString["lat"] : "null");
		return;
...
";
	...
	protected void cbSumbit_Callback(object source, CallbackEventArgs e)
	{
		var email = (tbEmail.Value == null ? "" : tbEmail.Value.ToString()).Trim();
		var notes = (mNotes.Value == null ? "" : mNotes.Value.ToString()).Trim(); 
		if (string.IsNullOrEmpty(email))
		{
			e.Result = "Потрібно вказати свій email";
			return;
		}
		if (!email.Contains("@"))
		{
			e.Result = "Вказано невірний email";
			return;
		}

		var lon = currentLogLat["lon"] as double?;
		var lat = currentLogLat["lat"] as double?;
		if (lon == null || lat == null)
		{
			e.Result = "Не вказані координати";
			return;
		}
		...
		try
		{
			...
			var to = WebConfigurationManager.AppSettings["ZayavkaOranda.EmailTo"];
			var to2 = WebConfigurationManager.AppSettings["ZayavkaOranda.EmailTo2"];
			if (string.IsNullOrEmpty(to2)) to2 = null;
			var baseurl = WebConfigurationManager.AppSettings["ZayavkaOranda.BaseUrl"];
			var url = baseurl + @"/Reports1NF/QueryNewObject.aspx?lat=" + lat.ToString().Replace(",",".") + "&lon=" + lon.ToString().Replace(",", ".");
			var subject = "ЗАЯВА ПОТЕНЦІЙНОГО ОРЕНДАРЯ";
			var body =
@"О {{datetime}} було зроблено заявку від потенційного орендаря на приміщення 
{{url}}
...
";
			body = body
				...
				.Replace("{{url}}", url)
				.Replace("{{email}}", email)
				.Replace("{{notes}}", !string.IsNullOrEmpty(notes) ? "Примітки: " + notes : "");

			EmailNotifier.SendMailMessage(from, to, to2, null, subject, body);
		}
		catch(Exception ex)
		{
			e.Result = "Надсилання заявки не вдалося. Спробуйте за кілька хвилин";
			return;
		}

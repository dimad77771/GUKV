<%@ Page Language="C#" AutoEventWireup="true" CodeFile="OrgBalansDeletedObject.aspx.cs" Inherits="Reports1NF_OrgBalansDeletedObject"
    MasterPageFile="~/NoHeader.master" Title="Картка Відчуженого Об'єкту" %>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceOrgSearch" runat="server" 
    ...
    SelectCommand="SELECT id, zkpo_code + ' - ' + full_name AS 'search_name' FROM organizations WHERE (zkpo_code LIKE @zkpo AND full_name LIKE @fname AND
        (is_deleted IS NULL OR is_deleted = 0) AND master_org_id IS NULL AND (SELECT vidch_org_id FROM reports1nf_balans_deleted WHERE id = @bal_id AND report_id = @rep_id) <= 0) or ((id = (SELECT vidch_org_id FROM reports1nf_balans_deleted WHERE id = @bal_id AND report_id = @rep_id)) AND ((SELECT vidch_org_id FROM reports1nf_balans_deleted WHERE id = @bal_id AND report_id = @rep_id) > 0))"
                ...
                <dx:PanelContent ID="PanelContent1" runat="server">
                    ...
                    <table border="0" cellspacing="0" cellpadding="2" width="810px">
                        <tr>
                            ...
                            <td>
                                <dx:ASPxComboBox ID="ComboVidchType" runat="server" ValueType="System.Int32" TextField="name" ValueField="id" Width="270px" 
                                    ...
                                    Title="Спосіб відчуження об'єкту">
                        ...
                        <tr>
                            ...
                            <td>
                                <dx:ASPxSpinEdit ID="EditVidchSquare" runat="server" NumberType="Float" Value='<%# Eval("sqr_total") %>' Width="270px" Title="Площа відчуження">
                        ...
                        <tr>
                            ...
                            <td>
                                <dx:ASPxTextBox ID="EditObjBtiReestrNoDeleted" runat="server" Text='<%# Eval("reestr_no") %>' Width="270px" Title="Інвентарний номер об'єкту" MaxLength="18">
                                    <ValidationSettings Display="None" ValidationGroup="MainGroup"></ValidationSettings>
                                </dx:ASPxTextBox>
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr>
                            ...
                            <td>
                                <dx:ASPxComboBox ID="ComboVidchDocKind" runat="server" ValueType="System.Int32" TextField="name" ValueField="id" Width="350px" 
                                    ...
                                    Title="Тип документу, що підтверджує відчуження об'єкту">
                                    <ValidationSettings Display="None" ValidationGroup="MainGroup"> <RequiredField IsRequired="false" /> </ValidationSettings>
                                </dx:ASPxComboBox>
                            </td>
                            ...
                            <td width="8px">&nbsp;</td>
                            <td>
                                <dx:ASPxTextBox ID="EditVidchDocNum" runat="server" Text='<%# Eval("vidch_doc_num") %>' Width="100px" Title="Номер документу, що підтверджує відчуження об'єкту" MaxLength="24">
                                    <ValidationSettings Display="None" ValidationGroup="MainGroup" EnableCustomValidation="true"></ValidationSettings>
                                </dx:ASPxTextBox>
                            </td>
                            ...
                            <td width="8px">&nbsp;</td>
                            <td>
                                <dx:ASPxDateEdit ID="EditVidchDocDate" runat="server" Value='<%# Eval("vidch_doc_date") %>' Width="100px" Title="Дата документу, що підтверджує відчуження об'єкту">
                                    <ValidationSettings Display="None" ValidationGroup="MainGroup" EnableCustomValidation="true"></ValidationSettings>
                                </dx:ASPxDateEdit>
                            </td>
                        </tr>

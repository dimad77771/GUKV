<%@ Page Language="C#" AutoEventWireup="true" CodeFile="RentedObjects_for_report_dbr_20250425b.aspx.cs" Inherits="Arenda_RentedObjects"
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceArendaObjects" runat="server" EnableCaching="false"
	...
	, F.total_free_sqr
	, F.free_sqr_korysna
	, case when F.id is null then '' when F.water = 1 then 'так' else 'ні' end as water
	, case when F.id is null then '' when F.heating = 1 then 'так' else 'ні' end as heating
	, (SELECT Q.name FROM dict_1nf_power_info Q where Q.id = F.power_info_id) as power
    , case when F.id is null then '' when F.gas = 1 then 'так' else 'ні' end as gas
	, (SELECT Q.name FROM dict_1nf_tech_stane Q where Q.id = F.free_sqr_condition_id) as tech_stane
	--, (SELECT left(Q.full_name, 150) FROM dict_rental_rate Q where Q.id = F.possible_using) as possible_using
	, F.possible_using
	, F.prozoro_number
    ...
    LEFT OUTER JOIN dict_history ON bb.history_id = dict_history.id
	LEFT JOIN reports1nf_balans bal on bb.id = bal.id
	OUTER APPLY 
	(
		select
		top 1
...
<%--        <dx:GridViewDataTextColumn FieldName="sqr_free_total" ReadOnly="True" ShowInCustomizationForm="True"
        ...
        <dx:GridViewDataTextColumn FieldName="history" ReadOnly="True" ShowInCustomizationForm="True"
            VisibleIndex="201" Visible="True" Caption="	Пам'ятка архітектури (історії)"></dx:GridViewDataTextColumn>

        <dx:GridViewDataTextColumn FieldName="F_id" Caption="Реєстра-ційний №" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="floor" Caption="Характеристика об’єкта оренди" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="total_free_sqr" Caption="Загальна площа об’єкта, кв.м." ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="free_sqr_korysna" Caption="Корисна площа об’єкта, кв.м." ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="water" Caption="Водопостачання" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="heating" Caption="Теплопостачання" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="power" Caption="Потужність електромережі" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="gas" Caption="Газопостачання" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="tech_stane" Caption="Технічний стан об’єкта" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="possible_using" Caption="Можливе використання вільного приміщення" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
        <dx:GridViewDataTextColumn FieldName="prozoro_number" Caption="Унікальний код обєкту у ЕТС Прозорро-продажі" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="210" Visible="True" />
    ...
    <SettingsCookies CookiesID="GUKV.ArendaObjects" Version="A222_37" Enabled="false" />

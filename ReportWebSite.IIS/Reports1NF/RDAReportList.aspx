<%@ page language="C#" autoeventwireup="true" inherits="Reports1NF_RDAReportList, App_Web_rdareportlist.aspx.5d94abc0" masterpagefile="~/NoMenu.master" title="Перелік звітів 1НФ підпорядкованих організацій" %>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceRdaReports" runat="server"
    ...
    SelectCommand="SELECT rep.*,
        ...
        FROM view_reports1nf rep WHERE rep.org_sfera_upr_id = 4 AND rep.org_district_id = @distr_id">
    <SelectParameters>
...
<dx:ASPxGridView
    ...
    <Columns>
        <dx:GridViewDataTextColumn FieldName="id" ReadOnly="True" ShowInCustomizationForm="False" VisibleIndex="0" Caption="Картка Звіту">
            <DataItemTemplate>
                <%# "<center><a href=\"javascript:ShowReportCard(" + Eval("id") + ")\"><img border='0' src='../Styles/EditIcon.png'/></a></center>"%>
            </DataItemTemplate>
        ...
        <dx:GridViewDataTextColumn FieldName="org_zkpo" ReadOnly="True" ShowInCustomizationForm="True" VisibleIndex="2" Caption="Код ЄДРПОУ">
            <Settings AllowHeaderFilter="False" />
            <DataItemTemplate>
                <%# "<a href=\"javascript:ShowOrganizationCard(" + Eval("organization_id") + ")\">" + Eval("org_zkpo") + "</a>"%>
            </DataItemTemplate>
        </dx:GridViewDataTextColumn>

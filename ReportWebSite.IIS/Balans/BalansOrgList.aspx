<%@ page language="C#" autoeventwireup="true" inherits="Balans_BalansOrgList, App_Web_balansorglist.aspx.c41eb26c" masterpagefile="~/NoHeader.master" title="Перелік Балансоутримувачів" %>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceBalansOrganizations" runat="server" EnableCaching="true"
    ...
    SelectCommand="SELECT vo.*, tot.balans_obj_count, tot.sqr_balans_total, tot.sqr_balans_non_habit,
                    rent.payment_zvit_uah, rent.received_zvit_uah, rent.debt_total_uah FROM view_organizations vo
                    ...
                    WHERE ((@p_com_filter = 0) OR (@p_com_filter <> 0 AND vo.form_of_ownership_int IN (32,33,34))) AND
                        ((@p_rda_district_id = 0) OR (vo.sfera_upr_id = 4 AND vo.addr_distr_new_id = @p_rda_district_id))"

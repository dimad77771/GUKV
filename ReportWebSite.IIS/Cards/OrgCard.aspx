<%@ page language="C#" autoeventwireup="true" inherits="Cards_OrgCard, App_Web_orgcard.aspx.f411966a" masterpagefile="~/NoMenu.master" title="Картка Організації" validaterequest="false" %>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceOrgCardBalans" runat="server" 
    ConnectionString="<%$ ConnectionStrings:GUKVConnectionString %>" 
    SelectCommand="SELECT balans_id, district, street_full_name, addr_nomer, sqr_total, purpose, history, sqr_in_rent, is_deleted FROM view_balans_all
        WHERE (organization_id = @org_id) AND (@show_del = 1 OR (@show_del = 0 AND (is_deleted IS NULL OR is_deleted = 0 OR is_not_accepted = 1)))"
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceOrgCardArendaGiven" runat="server" 
    ...
    SelectCommand="SELECT arenda_id, district, street_full_name, addr_nomer, org_renter_full_name, rent_square, agreement_num, rent_start_date, rent_finish_date, is_subarenda,
        purpose, object_name, agreement_active_int FROM m_view_arenda_agreements WHERE org_giver_id = @org_id AND (@show_del = 1 OR (@show_del = 0 AND agreement_active_int = 1))"
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceOrgCardOrendnaPlataByBalans" runat="server" 
    ConnectionString="<%$ ConnectionStrings:GUKVConnectionString %>" 
    SelectCommand="SELECT v.* FROM view_rent_by_balans_org v WHERE v.org_balans_id = @org_id AND
        NOT EXISTS (SELECT payment_id FROM view_rent_by_balans_org rbb WHERE rbb.org_balans_id = @org_id AND rbb.rent_period_id > v.rent_period_id)">
    <SelectParameters>
        <asp:Parameter DbType="Int32" DefaultValue="0" Name="org_id" />
    </SelectParameters>
</mini:ProfiledSqlDataSource>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceOrgCardOrendnaPlataByRenter" runat="server" 
    ConnectionString="<%$ ConnectionStrings:GUKVConnectionString %>" 
    SelectCommand="SELECT v.* FROM view_rent_by_renters v LEFT OUTER JOIN rent_renter_org renter ON renter.id = v.rent_renter_org_id
    WHERE renter.organization_id = @org_id AND NOT EXISTS (SELECT rent_id FROM view_rent_by_renters rbr WHERE renter.organization_id = @org_id AND rbr.rent_period_id > v.rent_period_id)">
    <SelectParameters>
        <asp:Parameter DbType="Int32" DefaultValue="0" Name="org_id" />
    </SelectParameters>
</mini:ProfiledSqlDataSource>
...
<dx:ASPxGridView ID="GridViewOrgCardBalans" ClientInstanceName="GridViewOrgCardBalans" runat="server" AutoGenerateColumns="False" 
    ...
    <Settings HorizontalScrollBarMode="Visible" ShowFooter="True" ShowFilterRow="True" ShowFilterRowMenu="True" ShowFilterBar="Visible" />
    <SettingsPager PageSize="10" />

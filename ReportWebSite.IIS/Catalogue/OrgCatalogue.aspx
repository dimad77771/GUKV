<%@ page language="C#" autoeventwireup="true" inherits="Catalogue_OrgCatalogue, App_Web_orgcatalogue.aspx.be391efe" masterpagefile="~/NoHeader.master" title="Каталог Організацій" %>
...
<mini:ProfiledSqlDataSource ID="SqlDataSourceAllOrganizations" runat="server" EnableCaching="true"
    ...
    SelectCommand="SELECT * FROM view_organizations WHERE ((org_deleted IS NULL) OR (org_deleted = 0)) AND LEN(COALESCE(full_name, '')) > 0 AND
        (@p_rda_district_id = 0 OR (sfera_upr_id = 4 AND addr_distr_new_id = @p_rda_district_id))"
